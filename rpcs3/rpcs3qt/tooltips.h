...
public:
	...
	const struct settings
	{
		...
		const QString output_scaling_mode = tr("Final image filtering. Nearest applies no filtering, Bilinear smooths the image, FidelityFX Super Resolution enhances upscaled images, and NVIDIA DLSS uses temporal super resolution when Vulkan and Streamline are available.\nDLSS consumes the rendered color, depth, and generated motion-vector inputs. The DLSS Preset selector exposes Streamline's letter presets A-O and sends the selected letter directly to every DLSS quality slot. If Streamline rejects that letter, RPCS3 retries native DLSS with preset K before using its local temporal guard. If those inputs or the vendor runtime are unavailable, RPCS3 falls back to its color-only scaler. Rendering jitter applies a deterministic subpixel camera offset before rasterization so temporal reconstruction can sample different phases; it is disabled by default and only targets validated camera-like passes. The optional motion-bias mask favors the current frame where the synthesized camera/depth motion disagrees, while the dynamic mask can suppress camera vectors at depth disocclusions. Far-depth rotation is a conservative sky/infinitely-far fallback, and the edge policy controls how off-screen history is handled. The experimental object-velocity option rerenders eligible single-target depth-tested draws with a camera-preserved previous packed guest transform block and composes their measured NDC delta into the temporal field; it is off by default because RPCS3 does not expose a native skinned-velocity attachment and some games keep camera and model transforms in the same constants. Mid-frame injection is a separate, conservative fullscreen substitution path. Frame generation uses the same tagged depth/MV inputs and can generate 1-5 frames per rendered frame; the optional raw-motion mode preserves pre-filter vectors for interpolation, while the default uses the more stable filtered field. The emulator has no separate HUD-less primitive.\nFidelityFX and DLSS can cause visual artifacts. Neither temporal path works with stereo 3D output for now.");
